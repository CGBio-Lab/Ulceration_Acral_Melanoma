---
title: "Ulceration Figure - 2 TF activity"
...
# Convert to df with gene identifiers
...
# Check how many TFs have at least 5 targets
...
This heatmap shows the activity of the top `r n_tfs` transcription factors across samples, grouped by ulceration status.
...
#saveRDS(TF_visualization, "top25_TFs_ulceration_vis.rds")
```


```{r, warning=FALSE, message=FALSE}
#colors <- rev(RColorBrewer::brewer.pal(n = 11, name = "RdBu")[c(2, 10)])
# Barplot of top differential TFs 
p1 <- ggplot(TF_visualization, aes(x = reorder(source, score), y = score)) + 
  geom_bar(aes(fill = score > 0), stat = "identity") +  # Discrete coloring based on sign
  scale_fill_manual(
    values = c("#0571b0", "#ca0020"),  # Blue for FALSE (negative), Red for TRUE (positive)
...
# Color based on consistency between mode of regulation and observed changes (t-value)
df <- df %>%
